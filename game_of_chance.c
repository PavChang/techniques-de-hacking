...
/*
   Pour exploiter ce programme, il faut faire attention que le moyen utilisé pour obtenir l'adresse de retour de la fonction jackpot

   br jackpot

   et puis observer en amont deux lignes pour éviter segmentaion fault (ajouter des adresses push $rbp dans le stack)
 */
...
long take_wager(int, int);
void play_the_game();
long pick_a_number();
long dealer_no_match();
long find_the_ace();
void fatal(char *);
...
long take_wager(int available_credits, int previous_wager) {
